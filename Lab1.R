...
NaNs come from log(-number). Inf comes from log3(Inf) and -Inf comes from log3(0).
...
#x_3_char = sample(rep(x_3, 25))
x_3_char = factor(sample(x_3, 100, replace = TRUE), levels = x_3, ordered = FALSE)
x_3_char
```
...
```{r}
x_3_ord = factor(x_3_char, ordered = TRUE)
```